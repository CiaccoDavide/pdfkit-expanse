...
type Padding = number | number[] | { top: number; right: number; bottom: number; left: number };
...
interface Options {
  ...
  padding?: Padding;
  absolutePosition?: boolean; //default false
...
export default class PDFDocumentWithTables extends PDFDocument {
  ...
  datasIndex: number | undefined;
  rowsIndex: number | undefined;

  constructor(option: PDFKit.PDFDocumentOptions) {
    super(option);
    this.opt = option;
  }
  ...
  addBackground(
    ...
    fillColor = "grey",
    fillOpacity = 0.1,
    callback = (_pdfDocument: PDFDocumentWithTables) => {}
  ) {
    // save current style
    this.save();
  ...
  table(
    table: Table,
    options: Options,
    callback: Function = (_pdfDocument: PDFDocumentWithTables) => {}
  ) {
    return new Promise((resolve, reject) => {
      try {
        ...
        let columnPositions: number[] = []; // 0, 10, 20, 30, 100
        let columnWidth = 0;
        ...
        const onFirePageAdded = () => {
          ...
          lockAddPage ||
            this.addPage({
              layout: this.page.layout as "portrait" | "landscape" | undefined,
              size: this.page.size,
        ...
        const separationsRow = (
          ...
          const divider = options?.divider ? options.divider[type] : {};
          // disabled
          const s = divider?.disabled || false;

          if (s === true) return;
          opacity = opacity || divider?.opacity || 0.5;
          width = width || divider?.width || 0.5;
          color = color || divider?.color || "black";
        ...
        // padding: 10,
        const prepareCellPadding = (p: Padding) => {
          // array
          if (Array.isArray(p)) {
            switch (p.length) {
              case 3:
          ...
          if (!options.hideHeader && table.headers.length > 0) {
            ...
            if (typeof table.headers[0] === "string") {
              ...
              table.headers.forEach((dataHeader, i) => {
                ...
                // deactivated temporarily, since eval is not recommended and header's renderers are still unused
